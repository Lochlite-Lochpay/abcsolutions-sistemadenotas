...
const form = useForm({
    ...
    client_id_accountings: props.company.client_id_accountings || '',
    client_key_accountings: props.company.client_key_accountings || '',
    client_audience_accountings: props.company.client_audience_accountings || '',
    access_token_accountings: props.company.access_token_accountings || '',
    personal_integration_id_accountings: props.company.personal_integration_id_accountings || '',
    api_generate_integration_id_accountings: props.company.api_generate_integration_id_accountings || '',
    accounting: props.company.accounting ? '1' : '0', // Convert boolean to string for checkbox         
});

const submit = () => {
    emit('toast', { type: 'connect', message: 'Iniciando conexão...' });
    form.put(route('dashboard.home.update', { home: props.company.id }), {
        onStart: () => {
            emit('toast', { type: 'connect', message: 'Atualizando empresa...' });
            console.log('Atualizando empresa...');
        },
        onSuccess: () => {
            emit('toast', { type: 'success', message: 'Empresa atualizada com sucesso!' });
            console.log('Empresa atualizada com sucesso!');
        },
        onError: () => {
            emit('toast', { type: 'danger', message: 'Falha ao atualizar empresa!' });
    ...
    <AppLayout :toast="toast">
        <div class="container mx-auto p-4">
            ...
            <form @submit.prevent="submit" class="bg-white dark:bg-gray-700 dark:text-white p-6 rounded shadow-md">
                ...
                <div class="mb-4">
                    <label for="apiid" class="block text-gray-700 dark:text-white">API ID (Qive)</label>
                    <input type="password" id="apiid" v-model="form.api_id" class="dark:bg-gray-300 dark:text-gray-800 mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    <span v-if="form.errors.api_id" class="text-red-500 text-sm">{{ form.errors.api_id }}</span>
                </div>
                <div class="mb-4">
                    <label for="apikey" class="block text-gray-700 dark:text-white">API Key (Qive)</label>
                    <input type="password" id="apikey" v-model="form.api_key" class="dark:bg-gray-300 dark:text-gray-800 mt-1 block w-full border-gray-300 rounded-md shadow-sm">
