...
const submit = () => {
    ...
    form.post(route('dashboard.admin.users.store'), {
        onStart: () => {
            emit('toast', { type: 'connect', message: 'Salvando usuário...' });
            console.log('Salvando usuário...');
        },
        onSuccess: () => {
            emit('toast', { type: 'success', message: 'Usuário salvo com sucesso!' });
        ...
        <div class="container mx-auto p-4">
            ...
            <form @submit.prevent="submit" class="bg-white dark:bg-gray-700 dark:text-white p-6 rounded shadow-md">
                ...
                <div class="mb-4">
                    ...
                    <input type="password" id="password" v-model="form.password" class="dark:bg-gray-300 dark:text-gray-800 mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    <span v-if="form.errors.password" class="text-red-500 text-sm">{{ form.errors.password }}</span>
                </div>
                <div class="mb-4">
                    <label for="password_confirmation" class="block text-gray-700 dark:text-white">Confirme a senha</label>
                    <input type="password" id="password_confirmation" v-model="form.password_confirmation" class="dark:bg-gray-300 dark:text-gray-800 mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
