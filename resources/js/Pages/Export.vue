...
const paginationRange = computed(() => {
    const totalPages = bids.value.last_page;
    const currentPage = bids.value.current_page;
    const range = [];
...
const getPageUrl = (page) => {
    return `${bids.value.path}?page=${page}`;
};

const exportForm = useForm({ bidId: null });

const exportBid = async (bidId) => {
    emit('toast', { type: 'connect', message: 'Iniciando conexão...' });
    exportForm.bidId = bidId;
    exportForm.processing = true;

    try {
        const response = await axios.get(route('dashboard.export.index', { id: bidId }), {
             responseType: 'blob',
        ...
        if (response.status !== 200) {
            emit('toast', { type: 'danger', message: 'Erro ao exportar edital.' });
            return;
        }
        ...
        link.href = url;
        link.setAttribute('download', `bid_${bidId}.csv`);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);

        emit('toast', { type: 'success', message: 'Edital exportado com sucesso!' });
    } catch (error) {
        console.error('Erro na exportação:', error);
        emit('toast', { type: 'danger', message: 'Falha ao exportar edital!' });
    } finally {
...
const fetchbidsSearch = async () => {
    emit('toast', { type: 'connect', message: 'Buscando editais...' });
    formsearch.get(route('dashboard.export.index'), {
        onStart: () => {
            emit('toast', { type: 'connect', message: 'Buscando editais...' });
        ...
        onError: () => {
            emit('toast', { type: 'danger', message: 'Falha ao buscar editais!' });
            console.error('Falha ao buscar editais!');
        },
        onFinish: () => {
            formsearch.reset();
...
<template>
    <AppLayout :toast="toast">
        <template #header>
            ...
            <form class="w-full flex justify-between" @submit.prevent="fetchbidsSearch" v-if="bids?.data?.length">
                        ...
                        <div class="w-full md:w-1/2 flex justify-between items-center group">
                         <div class="relative z-0 w-full flex justify-center items-center group">
                             ...
                             <label for="floating_search" class="w-full peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Busque por editais...</label>
                         </div>
        ...
        <div class="w-full mt-4">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
                <template v-if="bids?.data?.length">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="bid in bids.data" :key="bid.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ bid.id }}</th>
                                <td class="px-6 py-4">{{ bid.name }}</td>
                                <td class="px-6 py-4">{{ new Date(bid.created_at).toLocaleDateString() }}</td>
                                <td class="px-6 py-4">{{ new Date(bid.updated_at).toLocaleDateString() }}</td>
                                <td class="flex px-6 py-4 text-right">
                                    <button 
                                        @click="exportBid(bid.id)" 
                    ...
                    <div class="mt-4 flex justify-center items-center" v-if="bids.prev_page_url || bids.next_page_url">
                        <button 
                            v-if="bids.prev_page_url" 
                            @click="fetchbids(bids.prev_page_url)" 
                        ...
                        <button 
                            ...
                            :key="page" 
                            @click="fetchbids(getPageUrl(page))" 
                            :class="['px-4 py-2 rounded', { 'bg-blue-500 text-white': page === bids.current_page, 'bg-gray-300 text-gray-700 hover:bg-gray-400': page !== bids.current_page }]"
                        >
                            {{ page }}
                        </button>
                        <button 
                            v-if="bids.next_page_url" 
                            @click="fetchbids(bids.next_page_url)" 
                            class="bg-gray-300 text-gray-700 px-4 py-2 rounded hover:bg-gray-400"
                        >
                            Próximo
                        </button>
